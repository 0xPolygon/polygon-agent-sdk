...
async function syncStateAndGetStorage({ walletName, chainId }) {
  ...
  const origin = process.env.SEQUENCE_DAPP_ORIGIN || 'https://agentconnect.staging.polygon.technology'
    ...
    try {
      ...
      if (tokens.length > 0) {
        try {
          ...
          const heldFeeTokens = tokens.filter((t) => t?.contractAddress && heldAddresses.has(t.contractAddress.toLowerCase()))
          // Among tokens the wallet holds, prefer native USDC, then any USDC symbol, then first held
          erc20Token = heldFeeTokens.find((t) => t.contractAddress.toLowerCase() === USDC_POLYGON)
            || heldFeeTokens.find((t) => t?.symbol === 'USDC')
            || heldFeeTokens[0] || null
        } catch {
