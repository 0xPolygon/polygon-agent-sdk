  ...
  globalThis.fetch = async (input, init = {}) => {
    ...
    try {
      ...
      try {
        ...
        resText = `[unreadable: ${e?.message || e}]`;
      }
...
export async function runDappClientTx({
  ...
  if (preferNativeFee) {
    ...
    try {
      ...
      // Native token has a zero contractAddress ("0x000...000") — must check explicitly.
      // Do NOT fall back to feeOptions[0]: that's often POL, which would cause a simulation
      // revert if the wallet has no native balance.
      const isNativeOpt = (o) => {
      ...
      // No native option found → fall through to ERC20 path below
    } catch {
  ...
  if (!feeOpt) {
    ...
    try {
      ...
      // Always prefer USDC, then any held ERC20, then any ERC20.
      ...
      if (tokens.length > 0) {
        try {
          ...
          // projectAccessKey is in scope from syncStateAndGetStorage destructuring
          ...
          const heldAddresses = new Set(
            (balRes?.balances || []).map((b) => b.contractAddress?.toLowerCase())
          );
          const heldFeeTokens = tokens.filter(
            (t) => t?.contractAddress && heldAddresses.has(t.contractAddress.toLowerCase())
          );
          // Among tokens the wallet holds, prefer native USDC, then any USDC symbol, then first held
          erc20Token =
            heldFeeTokens.find((t) => t.contractAddress.toLowerCase() === USDC_POLYGON) ||
            heldFeeTokens.find((t) => t?.symbol === 'USDC') ||
            heldFeeTokens[0] ||
            null;
        } catch {
        ...
        if (!erc20Token) {
          ...
          erc20Token =
            tokens.find((t) => t?.contractAddress?.toLowerCase() === USDC_POLYGON) ||
            tokens.find((t) => t?.contractAddress && t?.symbol === 'USDC') ||
            tokens.find((t) => t?.contractAddress) ||
            null;
        }
      }
      ...
      if (debugFee)
        console.error(JSON.stringify({ debug: 'getFeeTokens failed', error: e?.message }, null, 2));
    }
  }
